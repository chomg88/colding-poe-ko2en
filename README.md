...
- 프레임 문구 표(`아이템 종류:`, `타락됨`, `(내재)`, 아이템 종류 값)는 게임 클라이언트가 찍는 문구라 CSV 에 없습니다. `FRAME_KEY` / `FRAME_CLASS` / `FRAME_VAL` / `FRAME_FLAG` 에 손으로 채워 넣었고, 빠진 표기가 있으면 여기에 추가하면 됩니다.
...
- 고급 정보의 접사 태그(`피해`, `방어력` …)와 종류(`접두어 속성 부여` …)도 손표입니다. `ADV_TAG` / `ADV_KIND` 에 있습니다.
- **손표는 전부 두 벌입니다.** 파이썬 쪽 `tools/ko2en.py` 와 웹 쪽 `web/src/lib/ko2en/engine.js` 에 같은 표가 한 벌씩 있고, 한쪽만 고치면 CLI 와 사이트 결과가 갈립니다. 반드시 양쪽을 같이 고쳐야 합니다.
