...
**옵션 칸 끝의 정규식**은 그 옵션을 게임 창고 검색창에 넣을 문자열로 바꿔 복사한다. 표 위의
**통합 정규식**은 지금 표에 보이는 옵션 전부를 `|` 로 묶는다. 둘 다 수치는 걸지 않는다 —
그 옵션이 붙었는지만 보면 되고, 무엇을 담을지는 표에 걸린 기준 필터가 이미 정한다.

경로석과 방법이 다르다. 경로석은 값이 아이템에 찍힌 수치에서 나서 `율:` 같은 앞자리면 됐지만,
서판은 값이 옵션 문구에서 나고 그 문구가 길다. 그래서 **그 옵션을 갈라 주는 가장 짧은 조각**을
쓴다. 조각은 두 곳에서 온다.

- `web/src/lib/tablet/known.js` — 손으로 맞춘 86줄. 이게 먼저다.
- `web/src/lib/tablet/regex.js` 의 생성기 — 표에 없는 문구를 옵션 글월에서 뽑는다.

**표의 절반 이상(86개 중 52개)이 옵션 문구가 아니라 서판 이름 줄의 접사 이름을 노린다.**
문구만으로는 안 갈리는 자리가 있어서다 — `지도에 금고 1개 추가 등장`(존재형)과
`지도에 금고 #개 추가 등장`(범위 1~2)은 수치가 1이면 게임 글월이 글자까지 같다. 이름 줄은
`<접두어> <서판> <접미어>` 라 `골동` 과 `공간` 으로 갈린다.

그래서 **수치를 이어 붙이면 안 된다.** 이름 줄 글자와 옵션 줄 숫자를 `.*` 로 이으면 검색이
줄 단위일 때 영영 안 맞는다. 지금은 수치를 아예 안 걸어서 이 문제가 없다 — 나중에 걸고 싶어지면
그 항만 생성기의 문구 조각으로 물러서야 한다.

조각이 **겹치면 안 된다.** 통합 정규식이 `|` 로 잇기 때문에 한 항이 남의 옵션 노릇을 한다.
가져온 표에도 그런 줄이 둘 있어 고쳤다(`발.*희` 가 '발굴되는 … 희귀 몬스터' 를, `터.희` 가
'탐험 몬스터 희귀도' 를 같이 잡았다). 검증이 이걸 본다.

생성기 쪽에는 **알맹이가 두 글자 이상**이어야 한다는 조건이 있다. 유일성을 옵션 문구끼리만
겨루는데 게임에서 맞춰 보는 것은 아이템 전체(이름·종류·잔여 사용 횟수·설명)라, `를` · `피`
같은 한 글자는 엉뚱한 줄에 걸린다.

**이 기능 때문에 카탈로그를 다시 만들지 마라.** 조각은 브라우저에서 계산한다. 카탈로그를 다시
만들면 파일 이름의 해시가 바뀌고, 수집기가 `load_state` 에서 이름이 다른 것을 보고 모아 둔
서판 값을 통째로 버린다 — 한 바퀴 여섯 시간이다.

**리그가 바뀌면** 새 옵션이 표에 없어 생성기로 넘어간다. 검증이 몇 개가 표 밖인지 알려 주니,
넘어간 것은 이름 줄 조각을 찾아 `known.js` 에 채워 넣는다.

```bash
cd web && npm run check:regex     # 경로석·서판 둘 다 본다
```
...
**조합 키는 낮은 구간에만 둔다.** 품질 축 넷(효율·몬스터 희귀도·아이템 희귀도·무리 규모)은
높은 값에서 서로 안 붙는다 — 16등급 효율50%+ 이면서 몬희50%+ 인 매물이 0건이다. 그런데 각
축의 맨 아래 한두 구간끼리는 붙는다. 2026-09-16 에 붙는 자리를 전부 세고 값을 재 봤더니
둘로 갈렸다.

| | 조합 | 조합값 | 단독 최대 |
|---|---|---|---|
| 값이 뛴다 | 16등급 효율30+ & 몬희50+ | 24.5카오스 | 6.4 (효율30) |
| | 15등급 효율50+ & 몬희50+ | 60카오스 | 9 (효율50) |
| 제자리다 | 16등급 몬희30+ & 아희40+ | 3카오스 | 3 (아희40) |
| | 16등급 아희40+ & 무리25+ | 3카오스 | 3 |

값이 뛰는 것은 대체로 **효율이 낀 조합**이다. 그래서 `COMBOS` 에는 조합값이 4카오스 이상
이면서 두 축 각각의 단독값보다 **1.5배 이상**인 것만 둔다 — 제자리인 조합에 키를 두면 이미
있는 값을 한 번 더 적는 것뿐이고 한 바퀴 슬롯만 먹는다. 매물 40건 미만도 두지 않는다(한두 건
팔리면 값이 통째로 흔들린다).

**출현 확률은 조합에 넣지 않는다.** 다른 축과 제일 잘 붙지만(16등급 무리25+ & 출현100+ 가
4303건) 그건 출현 확률이 품질 굴림과 따로 놀기 때문이고, 실제로 값이 붙는 것은 130%+ 짜리
단독이다.

**59키(단독 43 + 조합 16) · 한 바퀴 44분.** 서판 265키와 같은 프로세스에서 번갈아 돈다. 따로
돌리면 두 프로세스가 거래소 IP 한도를 나눠 쓰게 돼 서판 쪽이 그만큼 느려진다. 실제로 조사하다
세 번 한도에 걸렸다 (56초·283초·296초 대기).

```bash
python3 tools/waystone_prices.py --list        # 물어볼 59키 목록
...
글자다. 검색창 한도가 250자라 짧게 끊었고, 가장 긴 조합이 72자다.

**이 앞자리는 값이 아니라 게임 글월에 묶여 있다.** 시즌이 바뀌어 글월이 달라지면 시세는
멀쩡한데 정규식만 조용히 안 맞는다. 구간을 손볼 때는 검증을 같이 돌린다.

```bash
cd web && npm run check:regex     # 임계값 10~999 × 값 0~999 대입 · 빈 항 · 250자 한도
```
...
**시세 파일 모양** — 키는 단독이 `<등급>:<축>:<구간>`, 등급 바닥이 `<등급>:base`, 조합이
`<등급>:<축><구간>+<축><구간>` 이다. 조합 목록도 `combos` 로 같이 실어 보낸다 — 등급마다
목록이 달라서, 화면은 둘을 합쳐 줄을 만들고 한쪽에만 있는 조합은 반대쪽을 흐린 `-` 로 둔다.

```json
{"schema":1, "league":"Forbidden Rites", "publishedAt":…, "updatedAt":…,
 "rates":{"divine":320,"chaos":39.7}, "progress":[23,59], "tiers":[16,15],
 "axes":[{"id":"eff","label":"몬스터 효율","unit":"%","bands":{"16":[30,50,70,86],"15":[…]}}],
 "combos":[{"tier":16,"key":"16:eff50+rare30","parts":[["eff",50],["rare",30]]}],
...
`tools/waystone_mine.py` 는 거래소에 걸어 둔 내 경로석을 계정으로 찾아, 수집기가 모은
구간별 시세로 하나씩 값을 매긴다. 결과는 `.cache/tablet/waystone-mine-report.html`
(git 제외) — 사이트에는 올리지 않는다. 브라우저에서는 못 한다(CORS).

```bash
python3 tools/waystone_mine.py --account "계정이름#1234"   # 처음 — 받아서 리포트
python3 tools/waystone_mine.py                        # 캐시 + 최신 시세로 다시 매긴다
python3 tools/waystone_mine.py --refresh              # 거래소에서 매물을 다시 받는다
```

**서판 쪽(`tablet_mine.py`)과 무엇이 다른가.** 서판은 값이 옵션 문구에서 나서 카탈로그로
문구를 맞춰야 했다. 경로석은 값이 아이템에 찍힌 수치에서 나므로 맞출 문구가 없다 —
`item.properties` 의 `[MonsterRarity|몬스터 희귀도]` 같은 줄을 읽으면 끝이다. 매물의
explicit 은 전부 몬스터 쪽 위험 옵션이라 값의 근거가 못 된다.

그래서 **구간표를 이 도구에 다시 적지 않는다.** 축·구간·조합은 시세 파일에 실려 오므로
그것을 쓴다(화면이 하는 것과 같다). 여기 적어 둔 것은 축 id 와 수치 이름을 잇는 표
하나뿐이고, 시세에는 있는데 이 표에 없는 축은 리포트 머리에 경고로 뜬다.

**어떻게 매기나.** 축 하나는 굴린 수치 이하 구간들 중 가장 비싼 중앙값이다(표본 열 건이라
역전이 있어 아래 구간까지 같이 본다 — 서판과 같다). 두 축이 다 닿는 조합 키가 있으면 그
값도 본다. 경로석 하나는 **max(등급 바닥, 가장 비싼 단독, 가장 비싼 조합)** 이다. 시세가
'이 수치 이상인 경로석' 검색이라 다른 축이 같이 굴러간 매물을 이미 포함한다.

구간은 '이 값 이상' 이라 45%를 굴려도 25+ 구간밖에 없으면 25+ 값으로 매긴다. 그 구간
매물 무리의 값이지 이 경로석만의 값이 아니다 — 구간 맨 위에 걸친 것은 실제로 조금 더 비싸다.

15·16등급만 값이 붙는다. 그 아래는 수집기가 아예 안 물어보므로 '추정 안 함' 이다. 부활
횟수와 부패는 값을 가르지 않아 표에 적어만 둔다.

**시세는 로컬과 `data` 브랜치 중 게시가 새것을 쓴다.** 서판 도구는 로컬을 먼저 보는데,
수집기를 다른 기계로 옮기고 나면 여기 남은 옛 파일로 계속 매기게 된다. 지웠는지 기억하는
대신 둘을 견줘 고른다. 리포트 머리에 어느 쪽을 읽었는지 적힌다.

**요청.** 수집기와 IP 한도를 나눠 쓴다. 검색 한 번(100개가 넘으면 희귀도, 그다음 아이템
레벨을 반씩 쪼갠다) + 상세는 10개에 한 번이다. 서판보다 훨씬 가볍다 — 종류마다 검색하지
않고 분류(`map.waystone`) 하나로 끝난다.

**공개 탭에 걸어 둔 경로석만 보인다.** 개인 창고에 있는 것은 이 방법으로 못 찾는다.
