...
**통합 정규식**은 지금 표에 보이는 옵션 전부를 `|` 로 묶어, 옵션마다 기준을 넘는 가장 싼
수치로 건다. 만드는 방식은 <https://poe2-tablet-market-tool.netlify.app/> 의 것을 옮겼다
(`web/src/lib/tablet/regex.js`).

경로석과 방법이 다르다. 경로석은 값이 아이템에 찍힌 수치에서 나서 `율:` 같은 앞자리면 됐지만,
서판은 값이 옵션 문구에서 나고 그 문구가 길다. 그래서 **그 옵션을 다른 옵션과 갈라 주는 가장
짧은 조각**을 찾는다 — 문구 86개가 중앙 2자로 갈린다(`템.희`, `^몬스`).

세 가지를 조심한다.

- **조각은 알맹이가 두 글자 이상이어야 한다.** 유일성을 옵션 문구끼리만 겨루는데, 게임에서
  맞춰 보는 것은 아이템 전체(이름·종류·잔여 사용 횟수·설명)다. `를` · `피` 같은 한 글자는
  옵션끼리는 갈려도 엉뚱한 줄에 걸리고, 통합 정규식은 `|` 로 잇기 때문에 헐렁한 항 하나가
  창고를 통째로 물들인다.
- **숫자 자리는 `\d+` 다.** `\d` 한 자리로 두면 '효율 15%' 같은 두 자리 값에서 떨어진다.
  조각이 숫자를 이미 삼켰으면 수치를 뒤에 붙이지 않고 그 자리에 끼워 넣는다.
- **감독관 서판의 금고·성소·에센스·혼백 `#개 추가 등장` 은 수치가 1일 때 안 갈린다.** 같은
  서판의 `… 1개 추가 등장`(존재형)과 게임 글월이 글자까지 같다. 수치 2면 갈린다.

**이 기능 때문에 카탈로그를 다시 만들지 마라.** 조각은 브라우저에서 계산한다(86개 · 60ms).
카탈로그를 다시 만들면 파일 이름의 해시가 바뀌고, 수집기가 `load_state` 에서 이름이 다른 것을
보고 모아 둔 서판 값을 통째로 버린다 — 한 바퀴 여섯 시간이다.
