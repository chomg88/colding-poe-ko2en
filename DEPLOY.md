...
| 카오스 | 카오스를 내고 디바인을 사는 장 (디바인 ÷) | 반대쪽 장(디바인·엑잘을 내고 카오스를 사는 쪽)은 호가가 허수뿐이다 |
| 바알 | 엑잘 장 바닥값 | 어디에도 모이는 자리가 없다. 싸서 허수 바닥을 집어도 크게 안 틀린다. 카오스·디바인 장은 1:1 허수뿐이다 |

비싼 화폐에 바닥값을 쓰면 안 된다 — 한 번 그렇게 했다가 카오스가 1엑잘로 잡혀 카오스
매물이 40분의 1로 환산됐다. 이 밖의 화폐로 걸린 매물은 세지 않는다(몇 건을 세었는지가
`값 낸 수`).

모이는 자리를 찾아도 그게 허수 묶음일 수 있다. 2026-09-15 에 디바인 장의 카오스 호가가
0.25 · 1 · 1 · 1 · 5 디바인뿐이라 1:1 묶음이 시세로 잡혔고, 카오스가 400엑잘, 한 번은
380만 엑잘로 게시됐다. 그래서 환율끼리 맞춰 본다(`SANE`) — 1 디바인이 3~40카오스,
1 카오스가 5~30바알을 벗어나면 새 값을 버리고 지난 값을 쓴다. 지난 값도 어긋나면 그 화폐를
환율표에서 빼고 그 화폐 매물은 세지 않는다. 틀린 환율로 세느니 안 세는 게 낫다.

**1:1 호가는 아예 안 읽는다.** 값이 비슷한 화폐 짝이 없는데(엑잘 1 · 바알 3 · 카오스 40 ·
디바인 400) 장마다 바닥에 1:1 이 박혀 있다. 바알 바닥에 1엑잘이 끼어 바알이 3분의 1로
잡힌 뒤로 `book()` 이 뺀다.
